package com.ragnar.server.statistics;

import com.ragnar.server.config.FieryConfig;
import com.ragnar.server.data.statics.PerformInfo;
...
public class LogAPIStatics {
    ...
    public Long getStartTime(Integer daytime) {
        ...
        if (daytime == null || daytime == 0) {
            StartTime = Long.valueOf(String.valueOf(DateTimeHepler.getTimesmorning(System.currentTimeMillis() / 1000)));
        } else if (daytime == 1) {
            StartTime = Long.valueOf(String.valueOf(DateTimeHepler.getTimesmorning(System.currentTimeMillis() / 1000 - 1 * 24 * 60 * 60)));
        } else if (daytime == 2) {
            StartTime = Long.valueOf(String.valueOf(DateTimeHepler.getTimesmorning(System.currentTimeMillis() / 1000 - 2 * 24 * 60 * 60)));
        } else if (daytime == 3) {
            StartTime = Long.valueOf(String.valueOf(DateTimeHepler.getTimesmorning(System.currentTimeMillis() / 1000 - 3 * 24 * 60 * 60)));
        } else if (daytime == 4) {
            StartTime = Long.valueOf(String.valueOf(DateTimeHepler.getTimesmorning(System.currentTimeMillis() / 1000 - 4 * 24 * 60 * 60)));
        } else if (daytime == 5) {
            StartTime = Long.valueOf(String.valueOf(DateTimeHepler.getTimesmorning(System.currentTimeMillis() / 1000 - 5 * 24 * 60 * 60)));
        } else if (daytime == 6) {
            StartTime = Long.valueOf(String.valueOf(DateTimeHepler.getTimesmorning(System.currentTimeMillis() / 1000 - 6 * 24 * 60 * 60)));
        }
        return StartTime;
    }
    ...
    public Map<String, Map<String, String>> getPerformShowList(Long start, Long end, String url) {
        ...
        for (Map.Entry<Integer, PerformInfo> hourmap : _performMap.get(url).entrySet()) {
            ...
            String datetime = DateTimeHepler.TimeStamp2Date(String.valueOf(hourmap.getKey()), "HH");
        ...
        if (_performMap.size() > 0) {
            ...
            for (Map.Entry<String, Map<Integer, PerformInfo>> ent : _performMap.entrySet()) {
                if (ent.getValue().size() > 0) {
                    ...
                    for (Map.Entry<Integer, PerformInfo> hourent : ent.getValue().entrySet()) {
                        if (hourent.getKey() >= DateTimeHepler.getCurrentTime() - fieryConfig.getKeepdataday() * 86400) {
                            continue;
                        }
            ...
            for(Map.Entry<String,ArrayList<Integer>> urlent:delMap.entrySet()){
                for(Integer key:urlent.getValue()){
                    ...
                    log.info("del out time url:" + urlent.getKey() + ",log create_time:" + DateTimeHepler.TimeStamp2Date(key.toString(), "yyyy-MM-dd HH:mm:ss"));
                }
