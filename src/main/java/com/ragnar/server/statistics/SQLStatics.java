...
import com.ragnar.server.data.statics.SqlStruct;
        ...
        if (_sqlMap.size() > 0) {
            ...
            for (Map.Entry<String, Map<Integer, SqlStruct>> ent : _sqlMap.entrySet()) {
                ...
                if (ent.getValue().size() > 0) {
                    for (Map.Entry<Integer, SqlStruct> hourent : ent.getValue().entrySet()) {
                        ...
                        //log.info("del out time sql log:" + ent.getKey() + ",log create_time:" + DateTimeHepler.TimeStamp2Date(hourent.getKey().toString(), "yyyy-MM-dd HH:mm:ss"));
                    }
                }
            ...
            for(Map.Entry<String,ArrayList<Integer>> sqlent:delSqlMap.entrySet()){
                for(Integer key:sqlent.getValue()){
                    ...
                    log.info("del out time sql log:" + sqlent.getKey() + ",log create_time:" + DateTimeHepler.TimeStamp2Date(key.toString(), "yyyy-MM-dd HH:mm:ss"));
                }
