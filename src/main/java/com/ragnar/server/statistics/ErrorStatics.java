...
import com.ragnar.server.data.statics.LogInfo;
import com.ragnar.server.util.DateTimeHepler;
import com.ragnar.server.util.SimHash;
...
public class ErrorStatics {
    ...
    private void addLogInfo(Map<Long, Map<String, LogInfo>> logMap, String logStr, String token, Long nowTime, String filePath, Integer line, SimHash hash) {
        long dayTime = DateTimeHepler.getTimesmorning(nowTime);
    ...
    public Map<String, Map<String, String>> getErrorData(Integer type, Long dayTime) {
        ...
        for (Map.Entry<String, LogInfo> ent : tempMap.get(dayTime).entrySet()) {
            ...
            data.put("starttime", DateTimeHepler.TimeStamp2Date(String.valueOf(ent.getValue().getStartTime()), "yyyy-MM-dd HH:mm:ss"));
            data.put("endtime", DateTimeHepler.TimeStamp2Date(String.valueOf(ent.getValue().getEndTime()), "yyyy-MM-dd HH:mm:ss"));
    ...
    public boolean DelOutTimeLog() {
        if (_errorLogMap.size() > 0) {
            ...
            for (Map.Entry<Long, Map<String, LogInfo>> ent : _errorLogMap.entrySet()) {
                if (ent.getKey() >= DateTimeHepler.getCurrentTime() - fieryConfig.getKeepdataday() * 86400) {
                    continue;
                }
                errorList.add(ent.getKey());
                log.info("del out time error log,log create_time:" + DateTimeHepler.TimeStamp2Date(ent.getKey().toString(), "yyyy-MM-dd HH:mm:ss"));
            }
        ...
        if (_alarmLogMap.size() >= 0) {
            ...
            for (Map.Entry<Long, Map<String, LogInfo>> ent : _alarmLogMap.entrySet()) {
                ...
                log.info("del out time alarm log,log create_time:" + DateTimeHepler.TimeStamp2Date(ent.getKey().toString(), "yyyy-MM-dd HH:mm:ss"));
            }
        ...
        if (_exceptionLogMap.size() > 0) {
            ...
            for (Map.Entry<Long, Map<String, LogInfo>> ent : _exceptionLogMap.entrySet()) {
                ...
                log.info("del out time exception log,log create_time:" + DateTimeHepler.TimeStamp2Date(ent.getKey().toString(), "yyyy-MM-dd HH:mm:ss"));
            }
